...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    this.registrationForm = this.fb.group({
      user_Name: ['', [Validators.required, Validators.pattern(/^[a-z0-9]{4,20}$/)]],
      first_Name: ['', [Validators.required, Validators.pattern(/^[a-zA-Z]{3,}$/)]],
  ...
  onSubmit() {
    ...
    const data = {
      ...
      default_Shipping_Address: this.registrationForm.controls['default_Shipping_Address'].value,
    ...
    this.http.post('https://localhost:7150/register', data, { headers })
    .subscribe({
      next: (response) => {
